...
from .model.kis_dynamic_settings import DynamicKISSettings
...
def check_key(exchange_name):
    ...
    if exchange_name in CRYPTO_EXCHANGES:
        ...
        if 1 <= kis_num <= 50:
            # 동적 KIS 설정 사용
            dynamic_settings = DynamicKISSettings(settings_dict)
            kis_settings = dynamic_settings.get_kis_settings(kis_num)
            ...
            raise Exception(f"KIS 번호는 1-50 범위여야 합니다. 현재: {kis_num}")
